...
export const api = {
  async getFileByHash(hash) {
    ...
    const files = await dbInstance.select('SELECT * FROM files WHERE hash = $1 LIMIT 1', [hash])
    if (files.length > 0) {
      let file = files[0]
  ...
  async updateFileChunkCursor(hash, chunk_cursor, updated_at) {
    const db = await getDB()
    await db.execute('UPDATE files SET chunk_cursor = $1, updated_at = $2 WHERE hash = $3', [
      chunk_cursor,
      updated_at,
      hash
    ])
  ...
  async localFileSaved(hash, chunk_cursor, updated_at) {
    const db = await getDB()
    await db.execute('UPDATE files SET chunk_cursor = $1, updated_at = $2, is_saved = $3 WHERE hash = $4', [
      chunk_cursor,
      updated_at,
      1,
      hash
  ...
  async remoteFileSaved(hash, updated_at) {
    const db = await getDB()
    await db.execute('UPDATE files SET updated_at = $1, is_saved = $2 WHERE hash = $3', [updated_at, 1, hash])
  },
  ...
  async getPrivateFileByEHash(ehash) {
    const dbInstance = await getDB()
    const private_chat_files = await dbInstance.select('SELECT * FROM private_chat_files WHERE ehash = $1 LIMIT 1', [
      ehash
    ])
  ...
  async addGroupFile(ehash, group_hash, hash, size) {
    const db = await getDB()
    await db.execute('INSERT INTO group_chat_files (ehash, group_hash, hash, size) VALUES ($1, $2, $3, $4)', [
      ehash,
      group_hash,
      hash,
      size
  ...
  async getGroupFileByEHash(ehash) {
    const dbInstance = await getDB()
    const group_chat_files = await dbInstance.select('SELECT * FROM group_chat_files WHERE ehash = $1 LIMIT 1', [ehash])
  ...
  async getFilesForManagement({ category, fileExt, page = 1, pageSize = 20 }) {
    ...
    switch (category) {
      case 'bulletin': {
        ...
        const rows = await db.select(`${selectCols} ${fromClause}${groupBy}${orderBy} LIMIT $1 OFFSET $2`, [
          pageSize,
          offset
        ])
        return rows.map((r) => ({ ...r, is_saved: Int2Bool(r.is_saved) }))
  ...
  async getAvatarStorageStats() {
    ...
    return result ? { count: result.count, totalSize: result.total_size || 0 } : { count: 0, totalSize: 0 }
  },

  // Management: get bulletin files with ownership classification
  // ownership: 'owned' = at least one referencing bulletin belongs to an address in accounts table
  //            'others' = all referencing bulletins belong to addresses NOT in accounts table
  async getBulletinFilesWithOwnership({ ownership, fileExt, page = 1, pageSize = 20 }) {
    const db = await getDB()
    const offset = (page - 1) * pageSize

    // Determine ownership filter via HAVING clause
    let havingClause = ''
    if (ownership === 'owned') {
      havingClause = `
        HAVING SUM(CASE WHEN b.address IN (SELECT address FROM accounts) THEN 1 ELSE 0 END) > 0`
    } else if (ownership === 'others') {
      havingClause = `
        HAVING SUM(CASE WHEN b.address IN (SELECT address FROM accounts) THEN 1 ELSE 0 END) = 0`
    }

    // Build ext filter + params
    let whereExt = ''
    let params = []
    if (fileExt) {
      whereExt = ' AND LOWER(bf.file_ext) = $1'
      params = [fileExt.toLowerCase()]
    }
    ...
    const query = `
      SELECT
        ...
        MAX(bf.file_ext) AS file_ext,
        COUNT(DISTINCT bf.bulletin_hash) AS total_refs
      FROM files f
      INNER JOIN bulletin_files bf ON f.hash = bf.file_hash
      INNER JOIN bulletins b ON bf.bulletin_hash = b.hash
      WHERE 1=1${whereExt}
      GROUP BY f.hash
      ${havingClause}
      ORDER BY f.updated_at DESC
  ...
  // Management: count bulletin files matching ownership filter (for pagination)
  async getBulletinFileCountWithOwnership({ ownership, fileExt }) {
    const db = await getDB()
    ...
    // Wrap to count the number of distinct file hashes passing the HAVING filter
    const wrappedQuery = `SELECT COUNT(*) AS count FROM (${query})`
    const [result] = await db.select(wrappedQuery, params)
    return result ? result.count : 0
  },

  // Delete only the "others" bulletin_files references for a given file_hash.
  // Returns { deletedRefs, remainingRefs } — if remainingRefs is 0, caller may clean files + disk.
  async deleteOthersReferences(fileHash) {
    const db = await getDB()
    // Delete bulletin_files rows where the bulletin's address is NOT in accounts
    const delResult = await db.execute(
      `DELETE FROM bulletin_files WHERE file_hash = $1 AND bulletin_hash IN (
        SELECT hash FROM bulletins b WHERE NOT EXISTS (SELECT 1 FROM accounts a WHERE a.address = b.address)
      )`,
      [fileHash]
    )
    // Count remaining references
    const remaining = await db.select('SELECT COUNT(*) AS c FROM bulletin_files WHERE file_hash = $1', [fileHash])
    return {
      deletedRefs: delResult.changes || 0,
      remainingRefs: remaining[0]?.c || 0
    }
  }
}
