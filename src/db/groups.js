...
export const api = {
  ...
  async getGroupByHash(hash) {
    const dbInstance = await getDB()
    const groups = await dbInstance.select('SELECT * FROM groups WHERE hash = $1 LIMIT 1', [hash])
    if (groups.length > 0) {
      let group = groups[0]
      try {
        group.member = JSON.parse(group.member)
  ...
  async updateGroupDelete(hash, delete_json) {
    const db = await getDB()
    const result = await db.execute('UPDATE groups SET delete_json = $1, deleted_at = $2 WHERE hash = $3', [
      JSON.stringify(delete_json),
      delete_json.Timestamp,
      hash
    ])
  ...
  async acceptGroupRequest(hash) {
    const db = await getDB()
    await db.execute('UPDATE groups SET is_accepted = $1 WHERE hash = $2', [1, hash])
  },

  // group message
  async getGroupSession(group_hash) {
    const dbInstance = await getDB()
    let msgs = await dbInstance.select('SELECT * FROM group_messages WHERE group_hash = $1 ORDER BY signed_at ASC', [
      group_hash
    ])
  ...
  async readGroupSession(group_hash) {
    const db = await getDB()
    await db.execute('UPDATE group_messages SET is_readed = $2 WHERE group_hash = $1', [group_hash, Bool2Int(true)])
  },
  ...
  async confirmGroupMessage(group_hash, address, sequence) {
    const db = await getDB()
    await db.execute('UPDATE group_messages SET is_confirmed = $1 WHERE group_hash = $2 AND address = $3 AND sequence <= $4', [Bool2Int(true), group_hash, address, sequence])
  },
  ...
  async getGroupMessageByHash(group_hash, hash) {
    const dbInstance = await getDB()
    const msgs = await dbInstance.select('SELECT * FROM group_messages WHERE group_hash = $1 AND hash = $2 LIMIT 1', [
      group_hash,
      hash
    ])
    return msgs.length > 0 ? groupMessage2Display(msgs[0]) : null
