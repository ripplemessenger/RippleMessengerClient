...
const MessageCard = ({ message, mode = 'private', isLastConfirmed = false }) => {
  ...
  const fileObjectType = mode === 'group' ? MessageObjectType.GroupChatFile : MessageObjectType.PrivateChatFile
    ...
    <div className={`flex ${isSelf ? 'ml-auto flex-row-reverse' : 'mr-auto flex-row'} animate-slideUp`}>
      ...
      <div className={`flex flex-col ${isSelf ? 'items-end' : 'items-start'} max-w-[75%]`}>
        <div className={`flex flex-row justify-between`}>
          <button
            className={`rounded-full px-2 py-0.5 border shrink-0 cursor-pointer transition-colors focus:outline-none ${
              isLastConfirmed
                ? 'bg-green-500 border-green-500 dark:bg-green-600 dark:border-green-600'
                : 'border-primary/30 dark:border-primary/40 hover:bg-primary/10 dark:hover:bg-primary/20 hover:border-primary/60 dark:hover:border-primary/50 hover:text-text-primary dark:hover:text-dark-text-primary'
            }`}
            onClick={() => {
        ...
        <div className={`${isSelf ? 'rounded-tr-none' : 'rounded-tl-none'} max-w-full`}>
          ...
          {message.is_object === true && message.content.ObjectType === MessageObjectType.Bulletin && (
            <BulletinLink
              ...
              sequence={message.content.Sequence}
              hash={message.content.Hash}
              sour_address={message[senderField]}
            />
          )}
          {message.is_object === true && message.content.ObjectType === fileObjectType && (
            <ChatFileLink
              address={message[senderField]}
