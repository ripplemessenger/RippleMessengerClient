const FormButton = ({ title, disabled = false, className = '', onClick, variant = 'primary', children }) => {
  const variants = {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`w-full mt-4 py-2.5 text-base font-semibold rounded-xl shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center ${variants[variant] || variants.primary} ${className}`}
      disabled={disabled}
    >
      {children}
      {title}
